...
CreateClientConVar("hg_fov","90",true,false,nil,90,120)
...
hook.Add( "CreateMove", "SetShootPos", function()
...
hook.Add("CreateMove", "view_limit", function(cmd)
	local lp = LocalPlayer()

	if lp:GetNWEntity("HeSpectateOn") then return false end
...
CalcView = function(ply,vec,ang,fov,znear,zfar)
	...
	if ply:Alive() and ply:GetNWBool("fake") and IsValid(ragdoll) then
		...
		LerpEyeRagdoll = LerpAngleFT(0.08,LerpEyeRagdoll,LerpAngle(0.4,eyeAngs,att.Ang))
	...
	if not RENDERSCENE then
		...
		if scope then
			...
			if pointshooting then
				ScopeLerp = LerpFT(0.05, ScopeLerp, 0.5)
			else
				ScopeLerp = LerpFT(0.05, ScopeLerp,1)
			end
		else
			ScopeLerp = LerpFT(0.05,ScopeLerp,0)
		end
	end

	fov = Lerp(ScopeLerp,fov,60)
	...
	if wep and weps[wep:GetClass()] then
		...
		if weaponClass == "weapon_remington870" then
            --Vector(3.8,4,0.65)
            vecWep = hand.Pos + hand.Ang:Up() * 4.4 - hand.Ang:Forward() * 4 + hand.Ang:Right() * 1.07
		...
		if weaponClass == "weapon_deagle" then
			--Vector(2.7,10,0.4)
			vecWep = hand.Pos + hand.Ang:Up() * 5.05 - hand.Ang:Forward() * 10 + hand.Ang:Right() * -0.3
			angWep = hand.Ang + Angle(1,-0.5,0)
		end
