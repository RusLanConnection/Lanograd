...
local panel
local panel_items, panel_items_created = {}, {}
net.Receive("inventory",function()
	...
	local Otrub = net.ReadBool()
	local no_delay = net.ReadBool()
	print(no_delay)

	local curweapon = net.ReadString()

	if items[curweapon] and table.HasValue(Gunshuy,curweapon) then items[curweapon] = nil end
	...
	function panel:OnRemove()		
		...
		for wep in pairs(items) do
			timer.Remove("invcooldown " .. wep)
		end

		table.Empty(panel_items)
		table.Empty(panel_items_created)
	...
	for wep in pairs(items) do
		local func = function()
			...
			if panel_items[wep] then return end

			if wep == lootEnt.curweapon then return end

			local button = vgui.Create("DButton",panel)
			panel_items_created[wep] = true
			button:SetPos(x,y)
			...
			button.DoRightClick = function()
				panel_items[wep] = true
				panel_items_created[wep] = nil
		...
		if panel_items_created[wep] or (no_delay and panel:ChildCount() == table.Count(items)) then
			func()
		else
			ButtonNum = ButtonNum + 1

			timer.Create("invcooldown " .. wep, ButtonNum * 1, 1, func)
		end
	end
