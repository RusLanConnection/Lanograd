hook.Add("HomigradDamage","Organs",function(ply,hitgroup,dmginfo,rag,armorMul,armorDur,haveHelmet)
    ...
    if dmginfo:GetDamage() >= 35 or (dmginfo:GetDamageType() == DMG_CRUSH and dmginfo:GetDamage() >= 6 and ent:GetVelocity():Length() > 700) then
        ...
        if brokenLeftArm then
            ply.LeftArm = math.max(0.6,ply.LeftArm - sub)
    ...
    if not rag or (rag and not dmginfo:IsDamageType(DMG_CRUSH)) then
        ...
        if huy then
            if not haveHelmet and ply.Organs['brain']!=0 and dmginfo:IsDamageType(DMG_BULLET) and not inf.RubberBullets then
