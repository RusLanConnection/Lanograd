...
["weapon_remington870"]=Vector(1,0.1,0),
...
["weapon_remington870"]=Vector(21,-1,0),
...
function SpawnWeapon(ply,clip1)
	...
	if !IsValid(ply.wep) and table.HasValue(Guns,ply.curweapon) then
		...
		if IsValid(rag) then
			...
			if ply.curweapon == "weapon_pipe" then
				local ang = ply.wep:GetAngles()
...
function FireShot(wep)
	...
	if wep.Clip<=0 then
		...
		if weptable.HoldType == "revolver" then
			wep.NextShot = CurTime() + (weptable.ShootWait * 2)
		else
			wep.NextShot = CurTime() + (weptable.ShootWait * 1.84)
		end
	...
	if wep.curweapon!="weapon_ak74" then
		wep:GetPhysicsObject():ApplyForceCenter(wep:GetAngles():Forward()*-damage*2+wep:GetAngles():Right()*VectorRand(-90,90)+wep:GetAngles():Up()*500)			--сделать зависимым от force потом
	else
		local ply = wep:GetOwner()
		local rag = ply:GetNWEntity("Ragdoll")
