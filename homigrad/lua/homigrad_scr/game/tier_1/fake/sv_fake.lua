...
function Faking(ply) -- функция падения
	...
	if not ply.fake then
		...
		if IsValid(ply:GetActiveWeapon()) then
			...
			if ply:GetActiveWeapon():GetClass() == "weapon_bat" then
				ply:DropWeapon1(ply:GetActiveWeapon())
				ply:SelectWeapon("weapon_hands")
			end
		
			if weapons.IsBasedOn(ply:GetActiveWeapon():GetClass(), "weapon_hg_melee_base") then 
				if ply:GetActiveWeapon().HoldTypeWep == "melee2" then
					ply:DropWeapon1(ply:GetActiveWeapon())
					ply:SelectWeapon("weapon_hands")
				end
			end
		...
		if ply.LeftLeg < 1 and ply.RightLeg < 1 then

			ply:ChatPrint("У тебя сломаны обе ноги.")
			
			return 

		end
		
		if ply.pain > 180 then 
...
hook.Add("DoPlayerDeath","blad",function(ply,att,dmginfo)
	...
	--выпадение melee
	if IsValid(ply:GetActiveWeapon()) then
		if weapons.IsBasedOn(ply:GetActiveWeapon():GetClass(), "weapon_hg_melee_base") or ply:GetActiveWeapon():GetClass() == "weapon_bat" then
			ply:DropWeapon1(ply:GetActiveWeapon())
			ply:SelectWeapon("weapon_hands")
		end
	end
	...
	if not IsValid(rag) then
		rag.curweapon = ply.curweapon
		--print("игрок", ply.curweapon)
		--print("рэгдолл", rag.curweapon)
	end
...
hook.Add("PostPlayerDeath","MatFix",function(ply)
	ply:SetSubMaterial()
end)
...
function Stun(Entity)
	if Entity:IsPlayer() then
		...
		timer.Create( "StunEffect"..Entity:EntIndex(), 0.1, 80, function()
			...
			local ply = RagdollOwner(Entity:GetNWEntity("Ragdoll"))
...
function PlayerMeta:CreateRagdoll(attacker,dmginfo) --изменение функции регдолла
	...
	rag:SetNWString("Nickname", self:GetNWString("Nickname"))
...
hook.Add("Player Think","FakeControl",function(ply,time) --управление в фейке
	...
	if !ply.Otrub then
		...
		if ply:KeyDown( IN_JUMP ) and (RopeCount > 0 or ((rag.IsWeld or 0) > 0)) and ply.stamina>45 and (ply.lastuntietry or 0) < CurTime() then
			...
			-- RopeCount = math.max(RopeCount - 0.1,0) 
			...
			if Try > (7*RopeCount) or ((rag.IsWeld or 0) > 0) then
				if RopeCount>1 or (rag.IsWeld or 0 > 0) then
					...
					if (rag.IsWeld or 0) > 0 then
						ply:ChatPrint("Осталось отбить: "..tostring(math.ceil(rag.IsWeld)))
		...
		if(ply:KeyDown(IN_ATTACK))then
            ...
            if !ply.FakeShooting and !ply.arterybleeding then
				...
				local shadowparams = {
					...
					maxangular=670,
					maxangulardamp=600,
					maxspeeddamp = 40 + math.max(CustomWeight[rag:GetModel()] or 20, 20),
					maxspeed=500,
					teleportdistance=0,
					deltatime=0.01,
				}
		...
		if(ply:KeyDown(IN_ATTACK2))then
			...
			if (!ply.suiciding or TwoHandedOrNo[ply.curweapon]) then
				if TwoHandedOrNo[ply.curweapon] and IsValid(ply.wep) then
					...
					phys:ComputeShadowControl(shadowparams)
					shadowparams.pos=shadowparams.pos + eyeangs:Forward() * -50 + eyeangs:Right() * -17
		...
		if(ply:KeyDown(IN_USE))then
			...
			local shadowparams = {
				...
				angle=angs,
				maxangulardamp=10,
				maxspeeddamp = math.max(CustomWeight[rag:GetModel()] or 20, 20) / 2, 
				maxangular=370,
				maxspeed=40,
				teleportdistance=0,
				deltatime=deltatime,
