...
local function send(ply,lootEnt,remove,no_delay)
	if ply then
		net.Start("inventory")
		...
		net.WriteBool(lootEnt.Otrub)
		net.WriteBool(no_delay)

		net.WriteString(lootEnt.curweapon or "")

		--print(lootEnt.curweapon)
		...
		for ply in pairs(lootEnt.UsersInventory) do
			...
			send(ply,lootEnt,remove,no_delay)
		end
	end
end
...
net.Receive("ply_take_item",function(len,ply)
	...
	if prekol[wep] and not ply:IsAdmin() then ply:Kill() return end
	...
	if ply:HasWeapon(wep) then
		...
		if IsValid(ply) and IsValid(actwep) then
