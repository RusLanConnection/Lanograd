...
hook.Add("HomigradDamage","ImpulseShock",function(ply,hitGroup,dmginfo)
	...
	if hitGroup == HITGROUP_RIGHTLEG or hitGroup == HITGROUP_LEFTLEG then
		if ply.dmgimpulse > 22 then timer.Simple(0,function() if not ply.fake then Faking(ply) end end) end
	end

	if hitGroup == HITGROUP_CHEST then
		if ply.dmgimpulse > 65 then timer.Simple(0,function() if not ply.fake then Faking(ply) end end) end
	end
	...
	if ply.LeftLeg < 1 and ply.RightLeg < 1 then
		timer.Simple(0,function() if not ply.fake then Faking(ply) end end)
	end
end)
