...
hook.Add("Player Think","homigrad-blood",function(ply,time)
	...
	if ply.Blood >= 0 then
		if ply.Organs["artery"] == 0 and (ply.arteriaThink or 0) < time then
			ply.arteriaThink = time + 0.1
			if not ply.holdingartery then
				ply.Blood = math.Clamp(math.max(ply.Blood - 10,0), 0, 10000)
				BloodParticle(neck,ent:GetAttachment(ent:LookupAttachment("eyes")).Ang:Forward() * 200)
			else
				ply.Blood = math.Clamp(math.max(ply.Blood - 2,0), 0, 5000)
	...
	if ply.Bloodlosing > 0 then
		...
		ply.Blood = math.Clamp(math.max(ply.Blood - ply.Bloodlosing / 2,0), 0, 10000)
