...
function SWEP:Holster()
	if SERVER then
		if self.HoldTypeWep == "melee2" then
...
function SWEP:PrimaryAttack()
	...
	if true then
		...
		if IsValid( tr.Entity ) and SERVER then
			...
			dmginfo:SetDamageForce( self:GetOwner():GetAimVector() * self.Primary.Force )
			...
			if tr.Entity:IsNPC() or tr.Entity:IsPlayer() then
				...
				if IsValid( tr.Entity:GetPhysicsObject() ) then
					...
					dmginfo:SetInflictor( self )
					dmginfo:SetDamagePosition( tr.HitPos )
					dmginfo:SetDamageForce( self:GetOwner():GetAimVector() * self.Primary.Force*7 )
			...
			if IsValid(tr.Entity:GetPhysicsObject()) then
				if tr.Entity:IsPlayer() then
					tr.Entity:SetVelocity(self:GetOwner():GetAimVector() * self.Primary.Force * 1.5)
				end
	
				tr.Entity:GetPhysicsObject():ApplyForceOffset(self:GetOwner():GetAimVector() * 5000 * 1, tr.HitPos)
				self:GetOwner():SetVelocity(-self:GetOwner():GetAimVector() * self.Primary.Force * .8)
			end

			local doorrand = math.random(1,4)
			if self.Primary.Force == 190 and IsDoor(tr.Entity) and doorrand == 1 then
				BlastDoors(tr.Entity)
			end
		end
...
hook.Add( "WeaponEquip", "SetActive", function( weapon, ply )
	timer.Simple( 0, function()
		if weapons.IsBasedOn(weapon:GetClass(), "weapon_hg_melee_base") or weapon:GetClass() == "weapon_bat" then
