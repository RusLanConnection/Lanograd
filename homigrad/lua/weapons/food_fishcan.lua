...
function SWEP:PrimaryAttack()
	...
	if(SERVER)then
		if self.poisoned then 
			...
			ply.stamina = ply.stamina + math.random(20, 50)

			timer.Create("food_blood_regen " .. ply:EntIndex(),6, 5, function()
				if IsValid(ply) then
					ply.Blood = math.Clamp(ply.Blood + 150, 0, 5000)
					ply:SetHealth(math.Clamp(ply:Health() + 5, 0, 200))
				end
			end)
		end
