...
local function ToggleMenu(toggle)
    if toggle then
		...
		if wep:GetClass() ~= "weapon_hands" then
			local subwepmenu1 = wepMenu:AddOption("Выкинуть",function()
				LocalPlayer():ConCommand("say *drop")
			end)
			subwepmenu1:SetIcon("icon16/cart_put.png")
		end
        if wep:Clip1() > 0 then
            local subwepmenu2 = wepMenu:AddOption("Разрядить",function()
                net.Start("Unload")
                net.WriteEntity(wep)
                net.SendToServer()
            end)
			subwepmenu2:SetIcon("icon16/arrow_rotate_clockwise.png")
			local subwepmenu3 = wepMenu:AddOption("Суицид",function()
				LocalPlayer():ConCommand("suicide")
			end)
			subwepmenu3:SetIcon("icon16/cancel.png")
		end
		if laserweps[wep:GetClass()] then
			local subwepmenu5 = wepMenu:AddOption("Вкл/Выкл Лазер",function()
			...
			subwepmenu5:SetIcon("icon16/bullet_red.png")
		end
		if wep:GetClass() == "weapon_deagle" then
			local subwepmenu6 = wepMenu:AddOption("Русская рулетка",function()
				LocalPlayer():ConCommand("hg_rolldrum")
			end)
			subwepmenu6:SetIcon("icon16/flag_red.png")
		end
		...
		local submenu1 = plyMenu:AddOption("Меню Брони",function()
            LocalPlayer():ConCommand("jmod_ez_inv")
        end)
		submenu1:SetIcon("icon16/briefcase.png")
		local submenu2 = plyMenu:AddOption("Меню Патрон",function()
			LocalPlayer():ConCommand("hg_ammomenu")
		end)
		submenu2:SetIcon("icon16/gun.png")
		local submenu3 = plyMenu:AddOption("Вызвать рвоту",function()
			LocalPlayer():ConCommand("hg_blevota")
		end)
		submenu3:SetIcon("icon16/monkey.png")
		local submenu4 = plyMenu:AddOption("Встать/Упасть",function()
			LocalPlayer():ConCommand("fake")
		end)
		submenu4:SetIcon("icon16/user.png")
		local EZarmor = LocalPlayer().EZarmor
		if JMod.GetItemInSlot(EZarmor, "eyes") then
			local submenu5 = plyMenu:AddOption("Активировать Маску/Забрало",function()
				LocalPlayer():ConCommand("jmod_ez_toggleeyes")
			end)
			submenu5:SetIcon("icon16/eye.png")
		end
    else
