...
function homicide.StartRound(data)
    ...
    if SERVER then
        ...
        if roundType == 0 or roundType > 4 then
...
local red,blue = Color(200,0,10),Color(75,75,255)
local roundTypes = {"Чрезвычайное Положение", "Стандарт", "Зона Свободная От Оружия", "Дикий Запад"}
...
function homicide.HUDPaint_RoundLeft(white2)
    ...
    if startRound > 0 and lply:Alive() then
        ...
        draw.DrawText( "Вы " .. name, "HomigradFontLarge", ScrW() / 2, ScrH() / 2.2, Color( color.r,color.g,color.b,math.Clamp(startRound - 0.5,0,1) * 255 ), TEXT_ALIGN_CENTER )
