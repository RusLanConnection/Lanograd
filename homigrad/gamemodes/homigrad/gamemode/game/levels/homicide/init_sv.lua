...
function homicide.addModel(model,sex,clothes)
...
homicide.addModel("male_01", "male",3)
homicide.addModel("male_02", "male",2)
homicide.addModel("male_03", "male",4)
homicide.addModel("male_04", "male",4)
homicide.addModel("male_05", "male",4)
homicide.addModel("male_06", "male",0)
homicide.addModel("male_07", "male",4)
homicide.addModel("male_08", "male",0)
homicide.addModel("male_09", "male",2)
homicide.addModel("female_01", "female",2)
homicide.addModel("female_02", "female",3)
homicide.addModel("female_03", "female",3)
homicide.addModel("female_04", "female",1)
homicide.addModel("female_05", "female",2)
...
function ent:IsUsingValidModel()

	local Mod = self:GetModel()

	for k,v in pairs(ValidModels) do
		local ValidModel = "models/player/group01/" .. v .. ".mdl"

        if ValidModel == Mod then 
            return true 
        end
	end
...
function SetCloth(ent, outfit)

	if not outfit then return end

	if not ent:IsUsingValidModel() then return end

    local Fe = string.find(ent:GetModel(), "female") and true or false

    if Fe then
        ent:SetSubMaterial(ent.ClothingMatIndex,"models/humans/female/group01/"..outfit)
    else
        ent:SetSubMaterial(ent.ClothingMatIndex,"models/humans/male/group01/"..outfit)
    end

	ent.ClothingType = outfit
end

function GenerateClothes(ent)
	local Type = table.Random(ValidClothes)

	ent:SetSubMaterial() -- reset
...
local function makeT(ply)
    ...
    if homicide.roundType == 1 then --Стейт оф емердженси
        ...
        ply:Give("weapon_hg_t_cyanid_capsule")
        ply:Give("weapon_trap")
        ply:Give("weapon_mask")
        ply:Give("weapon_jam")
        --ply:Give("weapon_radio")
    elseif homicide.roundType == 2 then -- Стандарт
        ply:Give("weapon_kabar")
        ...
        wep:SetClip1(0)
    elseif homicide.roundType == 3 then -- Ган фри зон 
        ply:Give("weapon_kabar")
...
COMMANDS.russian_roulette = {function(ply,args)
	...
	for i,plya in ipairs(player.GetListByName(args[1]) or {ply}) do
...
function homicide.StartRoundSV()
    ...
    local count = math.max(math.random(1,math.ceil(#players / 6)),1) - countT
    for i = 1,count do
...
function homicide.PlayerSpawn(ply,teamID)
    ...
    --Новый код на гребанную одежду!!!
    local RandModel = table.Random(PlayerModelInfoTable)

	--ply:SetModel(teamTbl.models[math.random(#teamTbl.models)])
    ply:SetModel("models/player/group01/" .. RandModel.model .. ".mdl")
    ply.ClothingMatIndex = RandModel.clothes

    for _, bg in ipairs(ply:GetBodyGroups()) do
        ply:SetBodygroup(bg.id, math.random(0,bg.num))
    end
...
function homicide.PlayerCanJoinTeam(ply,teamID)
    if ply:IsAdmin() then
        if teamID == 2 then ply.forceCT = nil ply.forceT = true ply:ChatPrint("Ты будешь за убийцу в следущем раунде") return false end
        if teamID == 3 then ply.forceT = nil ply.forceCT = true ply:ChatPrint("Ты будешь за гражданина в следущем раунде") return false end
    else
...
local uncommon = {"medkit","weapon_molotok","painkiller", "weapon_phone"}
