...
function wick.StartRoundSV()
    ...
    for i,ply in ipairs(player.GetAll()) do ply.roleT = false end
...
function wick.PlayerCanJoinTeam(ply,teamID)
    --[[if ply:IsAdmin() then
        if teamID == 2 then ply.forceCT = nil ply.forceT = true ply:ChatPrint("Ты будешь за Джона Уика в следущем раунде.") return false end
        if teamID == 3 then ply.forceT = nil ply.forceCT = true ply:ChatPrint("Ты будешь за наемника в следущем раунде.") return false end
    else
