...
SWEP.PrintName 				= "Самодельная взрывчатка"
...
function SWEP:PrimaryAttack()
    ...
    if (IsValid(ent) and blacklist[ent:GetClass()]) or (IsValid(ent) and weapons.IsBasedOn(ent:GetClass(), "weapon_base" )) then return end
    ...
    ent:EmitSound("buttons/button24.wav",60,50,1, CHAN_WEAPON)
...
function SWEP:SecondaryAttack()
    if not SERVER then return end
    local deleteend = self:GetOwner().bomb
...
function SWEP:DrawWorldModel()
    ...
    if not IsValid(self.mdl) then
        self.mdl = ClientsideModel("models/weapons/weapon_ied_arabic.mdl")
        self.mdl:SetNoDraw(true)
        self.mdl:SetModelScale(1)
    end
    self:CallOnRemove("huyhuy",function() self.mdl:Remove() end)
    local matrix = self:GetOwner():GetBoneMatrix(16)
    if not matrix then return end

    self.mdl:SetRenderOrigin(matrix:GetTranslation() + matrix:GetAngles():Forward() * 0.4 + matrix:GetAngles():Right() * 1.3 + matrix:GetAngles():Up() * -1.1)
    self.mdl:SetRenderAngles(matrix:GetAngles() + Angle(0,0,180))
    self.mdl:DrawModel()
end
...
function SWEP:DrawHUD()
    ...
    if not IsValid(ent) then
        ...
        if blacklist[ent:GetClass()] or weapons.IsBasedOn(ent:GetClass(), "weapon_base" ) then
            surface.SetDrawColor(Color(255, 151, 151))
            draw.NoTexture()
            Circle(traceResult.HitPos:ToScreen().x, traceResult.HitPos:ToScreen().y, 5 / frac, 32)
            draw.DrawText( "Невозможно положить бомбу сюда.", "TargetID", traceResult.HitPos:ToScreen().x, traceResult.HitPos:ToScreen().y - 40, color_white, TEXT_ALIGN_CENTER )
        else
