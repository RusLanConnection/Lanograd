		...
		if not self:GetFists() then
			...
			if Tr.Hit then
				if self:CanPickup(Tr.Entity) then
					local Size = math.max(1 - Tr.Fraction, 0.05)
					...
					col.a = Lerp(Size, 0, 255)
					draw.DrawText( Tr.Entity:IsPlayer() and Tr.Entity:GetNWString("Nickname") or "", "HomigradFontLarge", Tr.HitPos:ToScreen().x, Tr.HitPos:ToScreen().y + 30, col, TEXT_ALIGN_CENTER )
				else
					local Size = math.max(1 - Tr.Fraction, 0.05)
...
function SWEP:Deploy()
	...
	self:GetOwner():ManipulateBoneAngles( self:GetOwner():LookupBone( "ValveBiped.Bip01_R_Hand" ), Angle( 0,0,0 ) )
	self:GetOwner():ManipulateBoneAngles( self:GetOwner():LookupBone( "ValveBiped.Bip01_R_Forearm" ), Angle( 0,0,0 ))
	self:GetOwner():ManipulateBoneAngles( self:GetOwner():LookupBone( "ValveBiped.Bip01_R_Clavicle" ),Angle( 0,0,0 ))
...
function SWEP:ApplyForce()
	...
	if IsValid(phys) then
		...
		if self.CarryEnt:GetClass() == "prop_ragdoll" then
			...
			if bone == Rhand or bone == Lhand then
				mul = mul * 2
			else
				mul = mul * 3.5
			end
			
			if self:GetOwner():KeyPressed( IN_RELOAD ) and (bone == Rhand or bone == Lhand) then
				if not ply then
...
function SWEP:Think()
	...
	if IsValid(self.CarryEnt) or self.CarryEnt then
		...
		if self:GetOwner():KeyPressed(IN_USE) then
			if IsValid(self.CarryEnt) then
				self.CarryEnt:Use(self:GetOwner())
			end
		end
	end
