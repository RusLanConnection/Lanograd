...
SWEP.PrintName = "Классический нож"
SWEP.Instructions = "Тот самый нож из интересной игры. У него есть реальный прототип 'strider badlands bowie'"
...
function SWEP:DrawWeaponSelection( x, y, wide, tall, alpha )
	if !IsValid(DrawModel) then
		...
		local vec = Vector(55,55,50)
		local ang = Vector(-48,-48,-38):Angle()
...
function SWEP:PrimaryAttack()
	...
	if SERVER then
		...
		self:GetOwner().stamina = math.max(self:GetOwner().stamina - math.random(1,2),0)
	end
	...
	if true then
		...
		if IsValid( tr.Entity ) and SERVER then
			...
			if IsValid(Phys) then
				...
				Phys:ApplyForceOffset(self:GetOwner():GetAimVector() * 5000 * 0.5, tr.HitPos)
	...
	if SERVER and Tr.Hit then
		if IsValid(Tr.Entity) and Tr.Entity:GetClass()=="prop_ragdoll" or Tr.Entity:IsPlayer() then
