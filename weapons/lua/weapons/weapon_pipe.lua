		...
		if IsValid( tr.Entity ) and SERVER then
			...
			if IsValid(Phys) then
				if tr.Entity:IsPlayer() then
					tr.Entity:SetVelocity(self:GetOwner():GetAimVector() * self.Primary.Force * 0.8)
				end
	
				Phys:ApplyForceOffset(self:GetOwner():GetAimVector() * 5000 * 0.8, tr.HitPos)
